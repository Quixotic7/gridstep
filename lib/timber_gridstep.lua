...
function Timber.does_sample_exist(sample_id)
  return samples_meta[sample_id].num_frames > 0
end

function Timber.get_sample_name(sample_id)
  if samples_meta[sample_id].num_frames <= 0 then
    return samples_meta[sample_id].error_status or "No sample"
  else
    return params:string("sample_" .. sample_id)
  end
end
...
function Timber.UI.SampleSetup:redraw()
  ...
  elseif self.copy_active or self.copy_params_active then
    ...
    if samples_meta[self.sample_id].num_frames > 0 then
      ...
      info = string.format("%03d", self.sample_id).." "..info
...
Timber.UI.AmpModEnv = {}
Timber.UI.AmpModEnv.__index = Timber.UI.AmpModEnv

function Timber.UI.AmpModEnv.new(sample_id)
  local ampGraph = EnvGraph.new_adsr(0, 20, nil, nil, params:get("amp_env_attack_" .. sample_id), params:get("amp_env_decay_" .. sample_id),
    params:get("amp_env_sustain_" .. sample_id), params:get("amp_env_release_" .. sample_id), 1, "lin")

  ampGraph:set_position_and_size(20, 15, 128-6-20, 15)

  local modGraph = EnvGraph.new_adsr(0, 20, nil, nil, params:get("mod_env_attack_" .. sample_id), params:get("mod_env_decay_" .. sample_id),
    params:get("mod_env_sustain_" .. sample_id), params:get("mod_env_release_" .. sample_id), 1, "lin")

  modGraph:set_position_and_size(20, 40, 128-6-20, 15)

  local ampOptions = {
    {
      label = "ATK",
      paramName = "amp_env_attack_",
      tabId = 1,
    },
    {
      label = "DEC",
      paramName = "amp_env_decay_",
      tabId = 1,
    },
    {
      label = "SUS",
      paramName = "amp_env_sustain_",
      tabId = 2,
    },
    {
      label = "REL",
      paramName = "amp_env_release_",
      tabId = 2
    }
  }

  for i = 1, #ampOptions do
    ampOptions[i].showVal = false
    ampOptions[i].clockId = false
    ampOptions[i].prevVal = params:get(ampOptions[i].paramName .. sample_id)
  end

  local modOptions = {
    {
      label = "ATK",
      paramName = "mod_env_attack_",
      tabId = 3,
    },
    {
      label = "DEC",
      paramName = "mod_env_decay_",
      tabId = 3,
    },
    {
      label = "SUS",
      paramName = "mod_env_sustain_",
      tabId = 4,
    },
    {
      label = "REL",
      paramName = "mod_env_release_",
      tabId = 4
    }
  }

  for i = 1, #modOptions do
    modOptions[i].showVal = false
    modOptions[i].clockId = false
    modOptions[i].prevVal = params:get(modOptions[i].paramName .. sample_id)
  end
  ...
  setmetatable(Timber.UI.AmpModEnv, {__index = Timber.UI})
  setmetatable(env, Timber.UI.AmpModEnv)
  return env
end

function Timber.UI.AmpModEnv:set_sample_id(id)
  self.sample_id = id
  Timber.env_dirty = true
...
function Timber.UI.AmpModEnv:set_tab(id)
  self.tab_id = util.clamp(id, 1, 4)
end

function Timber.UI.AmpModEnv:enc(n, delta)
  if Timber.shift_mode then delta = delta * 0.1 end
  if self.tab_id == 1 then
    if n == 2 then
...
function Timber.UI.AmpModEnv:key(n, z)
  if z == 1 then
    if n == 2 then
      self:set_tab((4 + self.tab_id - 2) % 4 + 1)
    elseif n == 3 then
      self:set_tab(self.tab_id % 4 + 1)
    end
    Timber.views_changed_callback(self.sample_id)
  end
end

function Timber.UI.AmpModEnv:redraw()
  ...
  screen.level(self.tab_id <= 2 and 15 or 3)
  screen.move(2, 15+8)
  screen.text("AMP")
  ...
  -- local paramLabels = {"ATK", "DEC", "SUS", "REL"}
  -- local paramNames = {"amp_env_attack_", "amp_env_decay_", "amp_env_sustain_", "amp_env_release_"}

  for i, k in pairs(self.ampOptions) do
    local v = params:get(k.paramName..self.sample_id)

    if v ~= k.prevVal then
      k.prevVal = v

      if k.clockId then
        clock.cancel(k.clockId)
      end
    ...
    screen.level(self.tab_id == k.tabId and 15 or 3)

    screen.move(x + (i-1)*xSeg+xSeg*0.5, y)

    if k.showVal then
      screen.text_center(string.format("%.2f", v))
    else
      screen.text_center(k.label)
    end
  end
  ...
  for i, k in pairs(self.modOptions) do
    local v = params:get(k.paramName..self.sample_id)
  ...
  if Timber.env_dirty then
    self.ampGraph:edit_adsr(params:get("amp_env_attack_" .. self.sample_id), params:get("amp_env_decay_" .. self.sample_id),
      params:get("amp_env_sustain_" .. self.sample_id), params:get("amp_env_release_" .. self.sample_id))

    self.modGraph:edit_adsr(params:get("mod_env_attack_" .. self.sample_id), params:get("mod_env_decay_" .. self.sample_id),
      params:get("mod_env_sustain_" .. self.sample_id), params:get("mod_env_release_" .. self.sample_id))

    Timber.env_dirty = false
  end

  self.ampGraph:redraw()
  self.modGraph:redraw()
end
...
local function draw_matrix(cols, rows, data, index, shift_mode)
  ...
  if not Timber.shift_mode then
    for i = 1, #cols do
      ...
      screen.move(grid_left + (i - 1) * col, 20)
